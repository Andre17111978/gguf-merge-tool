...
try: import gguf; HAS_GGUF=True
except ImportError: HAS_GGUF=False

try: from tqdm import tqdm; HAS_TQDM=True
except ImportError: HAS_TQDM=False

try: from huggingface_hub import HfApi, hf_hub_url, snapshot_download; HF_AVAILABLE=True
...
MAX_RETRIES = 3
RETRY_DELAY = 2
VERIFY_CHUNK_SIZE = 16 * 1024 * 1024
MERGE_MONITOR_INTERVAL = 0.5
...
def find_executable(name, base_dir=None):
    ...
    if C.get('llama_path'):
        ...
        if p.exists():
            paths.append(p)
            paths.extend([p/'bin', p/'Scripts'])
    candidates = [name, f"{name}.exe"] if sys.platform == 'win32' else [name]
    for path in paths:
        if not path.exists(): continue
        for n in candidates:
            if (exe := path / n).exists(): return str(exe)
    return None
...
# ✅ СКЛЕЙКА: stdout в отдельном потоке + мониторинг файла
def merge_parts(parts, output_path, progress_callback, process_holder=None, cancel_event=None):
    ...
    while proc.poll() is None:
        ...
        with output_lock:
            if output_buffer:
                lines_to_process = output_buffer.copy()
                output_buffer.clear()
        ...
        if output_path.exists():
            try:
                ...
                if current_size > last_size and total_size > 0:
                    ...
                    pct = min(95, int((current_size / total_size) * 100))
                    if pct > last_progress:
                        last_progress = pct
                        progress_callback(pct)
            except OSError: pass
        
        time.sleep(MERGE_MONITOR_INTERVAL)
        ...
        try:
            with requests.get(url, headers=headers, stream=True, timeout=(CONNECT_TIMEOUT, READ_TIMEOUT)) as resp:
                ...
                if resp.status_code == 404:
                    raise RuntimeError("Файл не найден (HTTP 404).")
...
class GGUFDownloader:
    def __init__(self, root):
        ...
        self._build_ui()
        self._load_token()
        self._load_history()  # ✅ Загружаем историю
    ...
    def _load_history(self):
        try:
            history = Path("hf_history.txt").read_text().strip().split('\n')
            self.repo_entry['values'] = [h for h in history if h]
        except: pass

    def _save_history(self, repo):
        if not repo: return
        try:
            history = Path("hf_history.txt").read_text().strip().split('\n') if Path("hf_history.txt").exists() else []
            history = [h for h in history if h and h != repo]
            history.insert(0, repo)
            Path("hf_history.txt").write_text('\n'.join(history[:10]))  # Храним последние 10
            self.repo_entry['values'] = history[:10]
        except: pass

    # ✅ ЗАГРУЗКА ЛОКАЛЬНЫХ ФАЙЛОВ
    def _load_local_files(self):
        folder = filedialog.askdirectory(title="Выберите папку с GGUF файлами")
        if not folder: return
        
        class LocalGGUF:
            def __init__(self, path):
                self.rfilename = path.name
                self.size = path.stat().st_size
                self.lfs = None  # Нет хэша от API
        
        files = sorted([LocalGGUF(p) for p in Path(folder).glob("*.gguf")], key=lambda x: x.rfilename)
        if not files:
            messagebox.showinfo("Информация", "GGUF файлы не найдены в папке")
            return
    ...
    def _check_tool(self):
        base_dir = self.llama_entry.get().strip()
        if find_executable("llama-gguf-split", base_dir): return True
        msg = "llama-gguf-split не найден!\n\nУкажите путь к папке с llama.cpp,\nили скачайте с GitHub."
    ...
    def _build_ui(self):
        ...
        # ✅ Combobox вместо Entry для истории
        self.repo_entry = ttk.Combobox(rf, width=50)
        self.repo_entry.insert(0, "unsloth/Qwen3.5-122B-A10B-GGUF")
        ...
        ttk.Button(bf, text=" Локальные", command=self._load_local_files).pack(side="left", padx=2)  # ✅ Кнопка локальных файлов
    ...
    def _load_files(self):
        ...
        self._save_history(repo)  # ✅ Сохраняем в историю
    ...
    def _download_worker(self, sel, target):
        ...
        completed = 0; errors = []; lock = threading.Lock(); speed_data = {}
        ...
        def download_one(index, file_index):
            ...
            try:
                ...
                speed_data[filename] = {'last_bytes': 0, 'last_time': time.time(), 'speed': 0, 'total_bytes': actual_size}
                ...
                with lock:
                    completed += 1; pct = int((completed / total_files) * 100)
                ...
                error_msg = f"{filename}: {e}"; log.error(error_msg)
                with lock: errors.append(error_msg)
    ...
    def _update_progress(self, progress, filename, current, total, speed_str="", eta_str=""):
        ...
        status_parts = []
        if speed_str and speed_str != "✅": status_parts.append(speed_str)
        if eta_str: status_parts.append(f"ETA: {eta_str}")
        status = " | ".join(status_parts) if status_parts else ""
        if speed_str == "✅": status = "✅ Завершено"
    ...
    def _verify_hashes(self):
        ...
        self.status_label.config(text="⏳ Подготовка к проверке хэшей...")
        self.root.update()
    ...
    def _verify_worker(self, sel):
        ...
        errors = []; verified = 0; total = len(sel)
        total_size = sum(self.files[idx][1] for idx in sel)
        processed_bytes = 0; last_update = 0; update_threshold = 0.2
        self.root.after(0, lambda: self.status_label.config(text=f" Проверка {total} файлов..."))

        for i, idx in enumerate(sel, 1):
            filename, _, sha = self.files[idx]
            file_path = target / filename
            self.root.after(0, lambda n=filename, c=i, t=total: self.status_label.config(
                text=f"⏳ Хэш [{c}/{t}] {Path(n).name}..."))
            if not file_path.exists(): errors.append(f"{filename}: не найден"); continue
            if not sha: errors.append(f"{filename}: нет хэша (загружено локально)"); continue
            ...
            try:
                with open(file_path, "rb") as f:
                    while chunk := f.read(VERIFY_CHUNK_SIZE):
                        ...
                        if total_size > 0:
                            progress = min(100, (processed_bytes / total_size) * 100)
                            if progress - last_update >= update_threshold or progress >= 100:
                                last_update = progress
                                self.root.after(0, lambda p=progress: self.progress_bar.configure(value=p))
            except Exception as e:
                errors.append(f"{filename}: ошибка чтения ({e})"); continue
            if hasher.hexdigest().lower() == sha.lower(): verified += 1
            else: errors.append(f"{filename}: хэш не совпадает!")
        self.root.after(0, self._finish_verify, verified, total, errors)
    ...
    def _finish_verify(self, verified, total, errors):
        ...
        if verified == total:
            ...
            self.status_label.config(text=f"⚠️ {len(errors)} ошибок из {total}")
    ...
    def _start_merge(self):
        ...
        self.status_label.config(text="⏳ Запуск склейки...")
        self.root.update()
        ...
        def worker():
            try:
                def progress_callback(pct):
                    ...
                    self.root.after(0, lambda p=pct: self.status_label.config(text=f"⏳ Склейка: {p}%"))
                if merge_parts(parts, output_path, progress_callback, self.process_holder, self.cancel_event):
                    self.root.after(0, lambda: self.progress_bar.stop())
                    self.root.after(0, lambda: self.progress_bar.configure(mode='determinate'))
                    ...
                    self.root.after(0, lambda: self.status_label.config(text="️ Склейка не удалась"))
            except Exception as e:
    ...
    if files:
        ...
        for f in file_iter:
            ...
            def progress(downloaded, total):
                ...
                if pbar: pbar.update(downloaded - pbar.n)
            try:
...
def cli_merge(directory, output=None, force=False):
    ...
    print(f"[INFO] Найдено {len(parts)} частей")
    print(f"[INFO] Выходной файл: {output}")
